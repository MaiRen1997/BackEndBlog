...
1. loguru有7个日志等级，依次是trace、debug、info、success，warning、error、critical，严重程度逐级上升

2. `level的值，从debug开始，以10递增，特有的success为25`
...
1. 通常有些日志配置在项目开发中是固定的，故可以准备一些默认的日志配置，如下是具体的封装代码
...
1. 这里设置了三个日志输出sink，分别是
   1. 控制台：日志等级 `debug` 及以上
   2. 服务日志文件：日志等级 `INFO` 及以上
     - 日志文件最长保留 7 天
     - 每天 0 点新创建一个 log 文件
   3. 服务错误日志：日志等级 `Error` 及以上
     - 日志文件最长保留 30 天
     - 超过10 MB就自动新建文件扩充

#### setup_logging的使用

1. 函数 `setup_logging` 的参数主要是控制服务日志，其他两个都是用默认的配置就够了，再项目中配置日志就不用写一大堆的日志配置代码，而是简单提供日志目录就可以使用默认的日志配置，默认的配置不满足需求的话，也可以通过参数进行改变。如下是具体的使用
