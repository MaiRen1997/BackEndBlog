...
1. 在main方法中，写如下代码:
   ...
   1. 在main方法中，创建了一个名为app的fastAPI实例，所以，第一个参数是`main:app`
...
1. `root_path` 是 Uvicorn 中用于**设置应用根路径**的参数，主要作用是在应用前面挂载一个**路径前缀**

2. 如果设置` root_path="/api"`，则每个接口都会加上`/api`
   ...
   - ```python
     ...
     @app.get("/")
     def redirect():
         # 会重定向到 /api/hello（如果 root_path="/api"）
         return RedirectResponse(url="/hello")
     ```

   - 如果不设置 `root_path`，请求 `/api/hello` 时，FastAPI 可能无法正确匹配路由。

#### workers

1. 作用

   1. **启用多进程模式**：创建指定数量的工作进程，每个进程独立运行应用实例
   2. **利用多核 CPU**：每个进程可以运行在不同的 CPU 核心上，充分利用服务器资源
   3. **提高并发能力**：多个进程可以同时处理多个请求（不同于单进程内的异步并发）
...
2. 当你设置 `workers=4` 时
   ...
   2. ```text
      主进程
      ├── Worker 1 (CPU Core 1) → 处理请求
      ├── Worker 2 (CPU Core 2) → 处理请求
      ├── Worker 3 (CPU Core 3) → 处理请求
      └── Worker 4 (CPU Core 4) → 处理请求
      ```

#### factory

1. 注意：**工厂函数不能有参数**（**通过命令行启动时，代码启动不影响**）。如果需要参数，可以通过环境变量或全局配置来传递

   1. 命令行方式

      1. ```python
         uvicorn "main:create_app(dev=True)" --factory # 不能使用
         ```
...
2. 支持热重载时状态重置
   ...
   1. 开发时使用 `--reload` 参数，代码修改后服务器会重启。
   2. 使用普通方式时，重启会复用之前的应用状态；
   3. 使用工厂模式则每次都会重新执行 `create_app()` 函数，确保获得一个全新的、干净的应用实例

3. 多个应用实例：工厂函数可以被多次调用，每次返回独立的应用实例

   1. ```python
      # 可以创建多个不同配置的应用
      dev_app = create_app(env="dev")
      prod_app = create_app(env="prod")
      ```

4. 延迟初始化（Lazy Initialization）

   1. 依赖项（如数据库连接、缓存等）可以在工厂函数内部按需初始化，而不是在模块导入时就创建

5. 场景区分

   | 场景               | 推荐使用 | 原因                                           |
   | :----------------- | :------- | :--------------------------------------------- |
   | **开发调试**       | ✅ 是     | 配合 `--reload` 效果更好，每次重载都是全新实例 |
   | **单元测试**       | ✅ 是     | 测试隔离的需要                                 |
   | **简单单文件项目** | ❌ 否     | 直接使用 app 实例更简单                        |
   | **大型模块化项目** | ✅ 是     | 符合代码组织规范                               |
   | **需要动态配置**   | ✅ 是     | 工厂函数可以接受配置参数                       |
