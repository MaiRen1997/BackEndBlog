...
2. 流程分析
   1. 先执行Info的f1方法，
      ...
      3. print先不执行
   2. 根据MRO顺序，INFO的下一个类是Foo，所以执行Foo的f1方法
      1. 在Foo，遇到super和print, 先执行super().f1()，根据MRO跳转到Bar
      2. print先不执行
   3. 根据MRO顺序，Foo的下一个类是Bar
      1. 没有super()方法，所以直接print `in Bar`
      2. 然后执行器交到上一层，也就是Foo层
      3. Foo层打印`in Foo`
   4. Info层中的print得以执行，并打印数据
...
1. super(Foo,self).f1() 的含义：**从 Foo 之后开始：也就是Bar**
