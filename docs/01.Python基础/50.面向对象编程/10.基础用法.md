...
class MyClass:
    def __init__(self):
        self.name = "public"  # 公开属性
```

#### 受保护属性

**约定**：单下划线开头 `_value`
表示“受保护的”，类内部和子类可以访问，但外部不应直接访问（实际仍可访问）。

```python
class MyClass:
    def __init__(self):
        self._protected = "should be used internally"
```

#### 私有属性

**名称改写机制**：双下划线开头 `__value`
Python 会将其改名为 `_ClassName__value`，防止被子类意外覆盖，但仍可通过改名后的名称访问。
...
**约定**：双下划线开头和结尾 `__method__`
由 Python 解释器预定义，用于实现运算符重载、对象生命周期等。
